...
export const api: ElectronAPI = {
  ...
  quitApp: () => send("AppQuitApp"),
  killSidecar: () => invoke("AppKillSidecar"),
  ...
  createWindow: (id, placement, follow, followOffset) =>
    invoke("WindowCreate", {
      id,
      placement,
      ...(follow ? { follow } : {}),
      ...(followOffset ? { followOffsetX: followOffset.x, followOffsetY: followOffset.y } : {}),
    }),
  closeWindow: (id) => invoke("WindowClose", { id }),
  stopWindowFollow: (id) => invoke("WindowFollowStop", { id }),
  setWindowFollowVisible: (id, visible) => invoke("WindowFollowSetVisible", { id, visible }),
  isWindowFollowing: () => invoke("WindowIsFollowing"),
  tabBarAtCursor: (exclude) => invoke("WindowTabBarAtCursor", { exclude }),
  transferTab: (input) => invoke("WindowTransferTab", input),
  onAdoptTab: (cb) => listen("WindowTabAdopted", (event) => cb({ seed: event.seed, screenX: event.screenX })),
