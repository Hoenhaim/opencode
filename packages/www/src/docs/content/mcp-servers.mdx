...
| `codemode`    | No       | Set to `true` to expose the server's tools through Code Mode. Inherits `mcp.codemode`, which defaults to `false`. |
...
Choose short server names that remain unique after normalization. MCP tools are exposed directly to the model by default. When Code Mode is enabled, tools are grouped by the normalized server name and called through `execute`.

Set `codemode` to `true` on a server to opt in:
...
Set `mcp.codemode` to `true` to opt in for all servers. A server's own `codemode` setting takes precedence, so `false` keeps that server's tools direct:
...
The highest-priority explicit `mcp.codemode` setting wins, including project `opencode.json` overrides of global configuration. Changing only this default updates tool exposure without reconnecting servers. On connection, Code Mode adds `codemode=false` to remote URLs to request raw tools only when opted in; an existing query parameter is preserved.
