...
| `codemode` | No | Exposes tools through Code Mode when `true`. Inherits `mcp.codemode`, which defaults to `false`. |
...
Choose short server names that remain unique after normalization. When Code Mode is enabled, tools are grouped by the normalized server name:
...
MCP tools are exposed directly to the model by default. Set `codemode` to `true` on a server to group its tools under Code Mode:
...
Set `mcp.codemode` to `true` to opt in for all servers. A server's own setting takes precedence, so `false` keeps that server's tools direct:
...
The highest-priority explicit `mcp.codemode` setting wins, including project overrides of global configuration. Changing only this default updates tool exposure without reconnecting servers. On connection, Code Mode adds `codemode=false` to remote URLs to request raw tools only when opted in; existing query parameters are preserved.
